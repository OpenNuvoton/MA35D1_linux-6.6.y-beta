...
#define SRAM_ADDRESS_BOUNDARY	0x24020000
...
static int ma35d1_rpmsg_sendto(struct rpmsg_endpoint *ept, void *data, int len, u32 dst)
{
	return ma35d1_rpmsg_send(ept, data, len);
}

static int ma35d1_rpmsg_trysendto(struct rpmsg_endpoint *ept, void *data, int len, u32 dst)
{
	return ma35d1_rpmsg_trysend(ept, data, len);
}
...
static const struct rpmsg_endpoint_ops ma35d1_rpmsg_endpoint_ops = {
	...
	.sendto = ma35d1_rpmsg_sendto,
	.trysendto = ma35d1_rpmsg_trysendto,
...
struct ma35d1_rpmsg_priv *ma35d1_rpmsg_register(struct device *parent, struct device_node *node)
{
	...
	struct resource r;
	int ret, isddr;
	u32  share_mem_addr, share_mem_size;
	...
	if (node) {
		...
		if (ret == 0) {
			...
			priv->tx_mem_start_addr = memremap(share_mem_addr, share_mem_size, MEMREMAP_WC);
		...
		dev_err_probe(&priv->dev, -ENODEV, "Failed to parse share memory region!\n");
		return ERR_PTR(-ENODEV);
	}

	isddr = (share_mem_addr + share_mem_size) > SRAM_ADDRESS_BOUNDARY ? 1 : 0;
	priv->enable_v2_arch = 1;
	...
	ret = rpmsg_ctrldev_register_device(&ndev->rpdev);
	if (ret) {
	...
	dev_info(parent, "RPMsg v%d engine with %s shared memory supported.\n",
			priv->enable_v2_arch ? 2 : 1, isddr ? "DRAM" : "SRAM");
