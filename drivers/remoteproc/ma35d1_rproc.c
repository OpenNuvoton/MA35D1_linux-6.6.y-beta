...
static void *ma35d1_m4_rproc_da_to_va(struct rproc *rproc, u64 da, size_t len, bool *is_iomem)
{
	struct ma35d1_rproc *nproc = rproc->priv;
	void *va = NULL;
...
int ma35d1_rproc_elf_load_segments(struct rproc *rproc, const struct firmware *fw)
{
	...
	for (i = 0; i < ehdr->e_phnum; i++, phdr++) {
		...
		if ((da < nproc->sram_size) && ((da + filesz) <= nproc->sram_size)) {
			ptr = rproc_da_to_va(rproc, da, filesz, NULL);
			if (!ptr) {
			...
			ptr = rproc_da_to_va(rproc, da, sram_remaining, NULL);
			if (!ptr) {
				dev_err(dev, "bad phdr da 0x%x mem 0x%x\n", da, filesz);
				ret = -EINVAL;
				break;
...
static int ma35d1_rproc_probe(struct platform_device *pdev)
{
	...
	if (!node)
	{
		dev_err(dev, "Error: Missing \"memory-region\" property\n");
		goto free_rproc;
	}
