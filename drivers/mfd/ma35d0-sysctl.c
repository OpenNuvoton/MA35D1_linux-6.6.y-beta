...
EXPORT_SYMBOL(ma35d0_reg_unlock);
...
EXPORT_SYMBOL(ma35d0_reg_lock);

static int ma35d0_sysctrl_probe(struct platform_device *pdev)
{
	if (!pdev->dev.of_node) {
...
static int ma35d0_sysctrl_remove(struct platform_device *pdev)
{
...
static const struct of_device_id ma35d0_sysctrl_match[] = {
	{ .compatible = "nuvoton,ma35d0-sysctrl", },
	{}
};

static struct platform_driver ma35d0_sysctrl_driver = {
	.driver = {
		.name = "ma35d0-sysctrl",
		.of_match_table = ma35d0_sysctrl_match,
	},
	.probe = ma35d0_sysctrl_probe,
	.remove = ma35d0_sysctrl_remove,
};

builtin_platform_driver(ma35d0_sysctrl_driver);
