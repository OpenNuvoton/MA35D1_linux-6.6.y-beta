...
/*
 * MA35D0 Camera Capture Interface Controller (CCAP) driver
 *
...
static void ccap_reg_write(struct ccap_device *ccap_dev, unsigned int reg, u32 value)
{
			...
			struct ccap_framesize *sd_framesize)
{
	...
	ret = v4l2_subdev_call_state_try(ccap->entity.source, pad, set_fmt, &format);
	if (ret < 0)
		return ret;
...
static inline void ccap_schedule_next(struct ccap_device *ccap_dev, struct vb2_v4l2_buffer *vbuf)
{
	dma_addr_t addr1;
...
static int ccap_queue_setup(struct vb2_queue *vq, unsigned int *nbuffers,
...
static int ccap_buf_prepare(struct vb2_buffer *vb)
{
	...
	struct v4l2_pix_format *pix = &ccap_dev->pix;
	unsigned int bytes_per_line = (ccap_fmt[ccap_dev->pix_idx].bpp * pix->width) / 8;
...
static int ccap_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	ccap_dev->sequence = 0;
	ret = v4l2_device_call_until_err(&ccap_dev->v4l2_dev, 0, video, s_stream, 1);
	...
	if (ccap_dev->bus.flags & V4L2_MBUS_HSYNC_ACTIVE_HIGH)
		polarity = HSP_HI;
	else
		polarity = HSP_LO;

	if (ccap_dev->bus.flags & V4L2_MBUS_VSYNC_ACTIVE_HIGH)
		polarity |= VSP_HI;
	else
		polarity |= VSP_LO;

	if (ccap_dev->bus.flags & V4L2_MBUS_PCLK_SAMPLE_RISING)
		polarity |= PCLKP_HI;
	else
		polarity |= PCLKP_LO;
	...
	/* Enable CAP Interrupt */
	ccap_reg_write(ccap_dev, CCAP_CTL, ccap_reg_read(ccap_dev, CCAP_CTL) |
...
static void ccap_stop_streaming(struct vb2_queue *vq)
{
	...
	if ((ccap_reg_read(ccap_dev, CCAP_CTL) & ((1 << 6) | (1 << 5)))
		...
		value = ccap_reg_read(ccap_dev, CCAP_CTL) | ~((1 << 6) | (1 << 5));
		ccap_reg_write(ccap_dev, CCAP_CTL, value);
	}
		...
		for (i = 0; i < ARRAY_SIZE(ccap_formats); i++) {
			...
			for (j = 0; j < num_fmts; j++)
				if (sd_fmts[j]->fourcc ==
					...
					/* Already available */
					dev_dbg(ccap->dev, "Skipping fourcc/code: %4.4s/0x%x\n",
			...
			if (j == num_fmts) {
				...
				dev_dbg(ccap->dev, "Supported fourcc/code: %4.4s/0x%x\n",
			 ...
			 struct v4l2_capability *cap)
{
	...
	strscpy(cap->card, "MA35D0 CCAP Interface", sizeof(cap->card));
				...
				struct v4l2_format *fmt)
{
	...
	pix->sizeimage = pix->height * ((pix->width * ccap_fmt[pix_idx].bpp) >> 3);
	...
	case V4L2_PIX_FMT_GREY:
		...
		if (ccap_dev->sd_format->mbus_code == MEDIA_BUS_FMT_RGB565_2X8_LE)
			parameter |= INFMT_RGB565;
		else if (ccap_dev->sd_format->mbus_code == MEDIA_BUS_FMT_YUYV8_2X8)
		...
		value = (ccap_reg_read(ccap_dev, CCAP_CWSP) & ~(CWSP_CWSADDRV | CWSP_CWSADDRH)) |
		...
		value = (ccap_reg_read(ccap_dev, CCAP_PKTSL) &
		...
		value = (ccap_reg_read(ccap_dev, CCAP_PKTSM) &
		...
		if (pix->pixelformat != V4L2_PIX_FMT_GREY) {
			value = (ccap_reg_read(ccap_dev, CCAP_STRIDE) &
				~STRIDE_PKTSTRIDE) | ((pix->bytesperline / 2) << 0);
			ccap_reg_write(ccap_dev, CCAP_STRIDE, value);

		} else {
			value = (ccap_reg_read(ccap_dev, CCAP_STRIDE) &
				~STRIDE_PKTSTRIDE) | ((pix->bytesperline / 1) << 0);
			ccap_reg_write(ccap_dev, CCAP_STRIDE, value);
		}
	...
	case V4L2_PIX_FMT_YUV420M:
		...
		value = (ccap_reg_read(ccap_dev, CCAP_PAR) & ~(1 << 7)) | parameter;
		ccap_reg_write(ccap_dev, CCAP_PAR, value);
		/* Set_Cropping start position for sensor */
		value = (ccap_reg_read(ccap_dev, CCAP_CWSP) &
		...
		value = (ccap_reg_read(ccap_dev, CCAP_PLNSL) & ~(PLNSL_PLNSVNL | PLNSL_PLNSVML)) |
			(((pix->height / u32GCD0) & 0xff) << 24 |
			((ccap_dev->fmt.fmt.pix.height / u32GCD0) & 0xff) << 16);
		...
		value = (ccap_reg_read(ccap_dev, CCAP_PLNSM) & ~(PLNSM_PLNSVNH | PLNSM_PLNSVMH)) |
			(((pix->height / u32GCD0) >> 8) << 24 |
			 ((ccap_dev->fmt.fmt.pix.height) >> 8) / u32GCD0 << 16);
		...
		value = (ccap_reg_read(ccap_dev, CCAP_PLNSL) & ~(PLNSL_PLNSHNL | PLNSL_PLNSHML)) |
			(((pix->width / u32GCD0) & 0xff) << 8 |
		...
		value = (ccap_reg_read(ccap_dev, CCAP_PLNSM) & ~(PLNSM_PLNSHNH | PLNSM_PLNSHMH)) |
			(((pix->width / u32GCD0) >> 8) << 8 |
			((ccap_dev->fmt.fmt.pix.width / u32GCD0) >> 8) << 0);
		ccap_reg_write(ccap_dev, CCAP_PLNSM, value);

		/* Frame Output Pixel Stride Width Register(Planar) */
		value = (ccap_reg_read(ccap_dev, CCAP_STRIDE) &
			~STRIDE_PLNSTRIDE) | ((pix->bytesperline / 2) << 16);
			ccap_reg_write(ccap_dev, CCAP_STRIDE, value);
	...
	/* Set CCAP Polarity */
	value = (ccap_reg_read(ccap_dev, CCAP_PAR) & ~(VSP_HI | HSP_HI | PCLKP_HI)) |
...
static irqreturn_t ccap_isr(int irq, void *dev_id)
{
	...
	if (!ccap_dev->active || list_empty(&ccap_dev->buf_list)) {
		if (printk_timed_ratelimit(&j, 500))
			dev_warn(ccap_dev->v4l2_dev.dev, "IRQ without active buffer: %x!\n",
				 irq_status);
	...
	if (!list_is_singular(&ccap_dev->buf_list)
		...
		vb = list_entry(ccap_dev->buf_list.next, struct ccap_buffer, list);
		ccap_schedule_next(ccap_dev, &vb->vb);
	...
	if (list_is_singular(&ccap_dev->buf_list)) {
		...
		struct ccap_buffer *new = list_entry(ccap_dev->active->list.next,
							struct ccap_buffer, list);
...
/* ma35d0_ccap display ioctl operations */
...
static int ccap_graph_init(struct ccap_device *ccap)
{
	...
	ep = of_graph_get_next_endpoint(ccap->dev->of_node, NULL);
	if (!ep) {
		dev_err(ccap->dev, "Failed to get next endpoint\n");
		return -EINVAL;
	}

	v4l2_async_nf_init(&ccap->notifier, &ccap->v4l2_dev);

	asd = v4l2_async_nf_add_fwnode_remote(&ccap->notifier,
					      of_fwnode_handle(ep),
	...
	if (IS_ERR(asd)) {
		dev_err(ccap->dev, "Failed to add subdev notifier\n");
		return PTR_ERR(asd);
	}

	ccap->notifier.ops = &ccap_sensor_ops;

	ret = v4l2_async_nf_register(&ccap->notifier);
	if (ret < 0) {
		dev_err(ccap->dev, "Failed to register notifier\n");
		v4l2_async_nf_cleanup(&ccap->notifier);
		return ret;
	}
...
static int ccap_probe(struct platform_device *pdev)
{
	...
	ccap_dev->bus.flags = ep.bus.parallel.flags;
	ccap_dev->bus.bus_width = ep.bus.parallel.bus_width;
	ccap_dev->bus.data_shift = ep.bus.parallel.data_shift;
	ccap_dev->bus_type = ep.bus_type;
...
static struct platform_driver __refdata ma35d0_ccap = {
	.probe = ccap_probe,
	.remove = ccap_remove,
	.driver = {
		   .name = "ma35d0-ccap",
...
module_platform_driver_probe(ma35d0_ccap, ccap_probe);

MODULE_DESCRIPTION("MA35D0 CCAP driver");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.0");
MODULE_ALIAS("platform:ma35d0-ccap");
