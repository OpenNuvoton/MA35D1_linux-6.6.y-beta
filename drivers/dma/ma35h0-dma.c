...
/*
 * This file contains a driver for the Nuvoton MA35H0 DMA engine
 * found on MA35H0
 *
...
static inline struct ma35h0_chan *to_ma35h0_dma_chan(struct dma_chan *c)
{
	return container_of(c, struct ma35h0_chan, vc.chan);
}

static inline struct ma35h0_desc *to_ma35h0_dma_desc(struct dma_async_tx_descriptor *t)
{
	return container_of(t, struct ma35h0_desc, vd.tx);
}

static void ma35h0_dma_desc_free(struct virt_dma_desc *vd)
{
	kfree(container_of(vd, struct ma35h0_desc, vd));
}

static int ma35h0_terminate_all(struct dma_chan *chan)
{
	struct ma35h0_chan *ch = to_ma35h0_dma_chan(chan);
	unsigned long flags;
	LIST_HEAD(head);
	u32 val;
	...
	if (ch->desc) {
		ma35h0_dma_desc_free(&ch->desc->vd);
		ch->desc = NULL;
	}
...
static struct dma_async_tx_descriptor *ma35h0_prep_dma_memcpy(
	...
	unsigned long tx_flags)
{
	...
	d->pcfg.reqsel = 0;
	ma35h0_set_channel_params(ch);
	ma35h0_set_transfer_params(ch, d->pcfg.reqsel);
	d->sg_addr =
	    dma_map_single(ch->dev, d->sg, sizeof(struct ma35h0_sg) * sg_len,
			   DMA_TO_DEVICE);
	writel(d->sg_addr,
...
static struct dma_async_tx_descriptor *ma35h0_prep_slave_sg(
	struct dma_chan *chan,
	...
	void *context)
{
	...
	if (ch->cfg.peripheral_size != sizeof(struct ma35h0_peripheral)) {
		dev_err(chan2dev(chan),
			"Invalid peripheral size %zu, expected %zu\n",
			ch->cfg.peripheral_size,
			sizeof(struct ma35h0_peripheral));
		return NULL;
	}
	...
	d->sg_addr = (u32) dma_map_single(ch->dev,
					  (void *)(d->sg),
					  sizeof(struct ma35h0_sg) * sg_len,
					  DMA_BIDIRECTIONAL);
	writel(d->sg_addr,
	       ch->base + (ch->ch_num * PDMA_OFFSET_CHAN_SIZE) +
	       PDMA_DSCT_NEXT);
...
static struct dma_async_tx_descriptor *ma35h0_prep_dma_cyclic(
	struct dma_chan *chan,
	dma_addr_t dma_addr,
	size_t buf_len,
	size_t period_len,
...
static struct dma_chan *ma35h0_of_xlate(struct of_phandle_args *dma_spec,
					struct of_dma *of)
{
	struct ma35h0_dmadev *edma = of->of_dma_data;
...
static int ma35h0_alloc_chan_resources(struct dma_chan *chan)
{
	struct ma35h0_chan *ch = to_ma35h0_dma_chan(chan);
...
static void ma35h0_free_chan_resources(struct dma_chan *chan)
{
	struct ma35h0_chan *ch = to_ma35h0_dma_chan(chan);
...
static void ma35h0_start_dma(struct ma35h0_chan *ch)
{
	struct ma35h0_desc *d = ch->desc;
	u32 ctrl;
...
static void ma35h0_dma_start_sg(struct ma35h0_chan *ch)
{
	ma35h0_set_dma_timeout(ch);
	ma35h0_start_dma(ch);
}

static void ma35h0_dma_start_desc(struct dma_chan *chan)
{
	struct ma35h0_chan *ch = to_ma35h0_dma_chan(chan);
	struct virt_dma_desc *vd;
	...
	list_del(&vd->node);
	ch->desc = to_ma35h0_dma_desc(&vd->tx);
	ma35h0_dma_start_sg(ch);

}

static void ma35h0_issue_pending(struct dma_chan *chan)
{
	struct ma35h0_chan *ch = to_ma35h0_dma_chan(chan);
	unsigned long flags;
	...
	if (vchan_issue_pending(&ch->vc) && !ch->desc)
		ma35h0_dma_start_desc(chan);
	spin_unlock_irqrestore(&ch->vc.lock, flags);
}

static enum dma_status ma35h0_tx_status(struct dma_chan *chan,
					dma_cookie_t cookie,
...
static irqreturn_t ma35h0_dma_interrupt(int irq, void *devid)
{
	int i;
	u32 val;
	struct ma35h0_dmadev *dmadev = devid;
	unsigned int intsts, tdsts;
	struct ma35h0_chan *ch = &dmadev->channels[dmadev->nr_chans - 1];
...
static void ma35h0_dma_release(struct dma_device *dma_dev)
{
	struct ma35h0_dmadev *edma =
		container_of(dma_dev, struct ma35h0_dmadev, ddev);

	put_device(edma->ddev.dev);
	kfree(edma);
}

static int ma35h0_probe(struct platform_device *pdev)
{
	struct clk *pdma_clk;
	...
	edma->ddev.device_prep_dma_memcpy = ma35h0_prep_dma_memcpy;
	edma->ddev.device_prep_slave_sg = ma35h0_prep_slave_sg;
	edma->ddev.device_prep_dma_cyclic = ma35h0_prep_dma_cyclic;
	edma->ddev.device_alloc_chan_resources = ma35h0_alloc_chan_resources;
	edma->ddev.device_free_chan_resources = ma35h0_free_chan_resources;
	edma->ddev.device_issue_pending = ma35h0_issue_pending;
	edma->ddev.device_tx_status = ma35h0_tx_status;
	edma->ddev.device_config = ma35h0_slave_config;
	edma->ddev.device_terminate_all = ma35h0_terminate_all;
	edma->ddev.device_release = ma35h0_dma_release;
	edma->ddev.dev = dev;
	...
	for (i = 0; i < nr_chans; i++, ch++) {
		...
		ch->vc.desc_free = ma35h0_dma_desc_free;
	...
	ret = devm_request_irq(dev, irq, ma35h0_dma_interrupt, 0,
			       pdev->name, edma);
	if (ret) {
	...
	ret = of_dma_controller_register(node, ma35h0_of_xlate, edma);
	if (ret) {
...
static int ma35h0_remove(struct platform_device *pdev)
{
	struct ma35h0_dmadev *m = platform_get_drvdata(pdev);
...
static const struct of_device_id ma35h0_dma_match[] = {
	{.compatible = "nuvoton,ma35h0-dma" },
	{ }
};

MODULE_DEVICE_TABLE(of, ma35h0_dma_match);

static struct platform_driver ma35h0_driver = {
	.probe = ma35h0_probe,
	.remove = ma35h0_remove,
	.driver = {
		   .name = "ma35h0-dma",
		   .of_match_table = ma35h0_dma_match,
		    },
};

static int ma35h0_init(void)
{
	return platform_driver_register(&ma35h0_driver);
}

subsys_initcall(ma35h0_init);

MODULE_AUTHOR("Shan-Chun Hung <schung@nuvoton.com>");
MODULE_DESCRIPTION("Nuvoton MA35H0 DMA controller driver");
