...
#define GPIO_PIN_DATA(base, pin) (*((unsigned int *)((base+0x800) + ((pin)<<2))))

char *gpio_port_name[] = {
	"gpioa", "gpiob", "gpioc", "gpiod", "gpioe", "gpiof", "gpiog", "gpioh",
	"gpioi", "gpioj", "gpiok", "gpiol", "gpiom", "gpion"
};
...
static int nvt_get_group_pins(struct pinctrl_dev *pctldev, unsigned int selector,
...
static int nvt_pinctrl_dt_node_to_map_func(struct pinctrl_dev *pctldev, struct device_node *np,
...
static void nvt_dt_free_map(struct pinctrl_dev *pctldev, struct pinctrl_map *map,
...
static const char *nvt_pinmux_get_func_name(struct pinctrl_dev *pctldev, unsigned int selector)
{
	struct nvt_pinctrl *npctl = pinctrl_dev_get_drvdata(pctldev);
...
static int nvt_pinmux_get_func_groups(struct pinctrl_dev *pctldev, unsigned int function,
...
static int nvt_pinmux_set_mux(struct pinctrl_dev *pctldev, unsigned int selector,
				unsigned int group)
{
	struct nvt_pinctrl *npctl = pinctrl_dev_get_drvdata(pctldev);
...
static int nvt_gpio_core_direction_out(struct gpio_chip *gc, unsigned int gpio_num, int val)
{
	unsigned long flags;
	unsigned int value;
...
static void nvt_gpio_core_set(struct gpio_chip *gc, unsigned int gpio_num, int val)
{
	struct nvt_pin_bank *bank = gpiochip_get_data(gc);
	void __iomem *base = bank->reg_base;
...
static int nvt_irq_irqtype(struct irq_data *d, unsigned int type)
{
	...
	if (type & IRQ_TYPE_LEVEL_MASK) {
		writel(readl((unsigned int *)(bank->reg_base + 0x18)) |
			(0x1 << num),
			(unsigned int *)(bank->reg_base + 0x18));
		writel(readl((unsigned int *)(bank->reg_base +
...
static int nvt_gpiolib_register(struct platform_device *pdev, struct nvt_pinctrl *npctl)
{
	...
	for (i = 0; i < ctrl->nr_banks; ++i, ++bank)  {
		...
		bank->chip.fwnode = of_fwnode_handle(bank->of_node);
		spin_lock_init(&bank->lock);
		if (bank->irq > 0) {
			struct gpio_irq_chip *girq = &bank->chip.irq;
...
static int nvt_get_bank_data(struct nvt_pin_bank *bank, struct nvt_pinctrl *npctl)
{
	struct resource res;
...
static int nvt_pinctrl_get_soc_data(struct nvt_pinctrl *pctl, struct platform_device *pdev)
{
	int i;
	...
	ctrl->pin_banks = devm_kcalloc(&pdev->dev, ctrl->nr_banks, sizeof(*ctrl->pin_banks), GFP_KERNEL);
	if (!ctrl->pin_banks)
		return -ENOMEM;
...
static void nvt_gpio_cla_port(unsigned int gpio_num, unsigned int *group, unsigned int *num)
{
	*group = gpio_num / MAX_GPIO_OFFSET;
	*num = gpio_num % MAX_GPIO_OFFSET;
}

static int nvt_pinconf_set_output(struct nvt_pinctrl *npctl, unsigned int pin_id, bool out)
{
...
static int nvt_pinconf_get_output(struct nvt_pinctrl *npctl, unsigned int pin_id)
{
...
static int nvt_pinconf_get_pull(struct nvt_pinctrl *npctl, unsigned int pin_id)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_set_pull(struct nvt_pinctrl *npctl, unsigned int pin_id, int pull_up)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_get_schmitt_enable(struct nvt_pinctrl *npctl, unsigned int pin_id)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_set_schmitt(struct nvt_pinctrl *npctl, unsigned int pin_id, int schmitt)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_get_slew_rate(struct nvt_pinctrl *npctl, unsigned int pin_id)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_set_slew_rate(struct nvt_pinctrl *npctl, unsigned int pin_id, int rate)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_set_power_source(struct nvt_pinctrl *npctl, unsigned int pin_id, int src)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_get_power_source(struct nvt_pinctrl *npctl, unsigned int pin_id)
{
	int port_num, group_num;
	unsigned int value;
	void __iomem *base;
...
static int nvt_pinconf_get_drive_strength(struct nvt_pinctrl *npctl, unsigned int pin_id,
					u16 *strength)
{
	int port_num, group_num;
	unsigned int value;
...
static int nvt_pinconf_set_drive_strength(struct nvt_pinctrl *npctl, unsigned int pin_id,
					int strength)
{
	int port_num, group_num;
	unsigned int value;
...
static int nvt_pinconf_get(struct pinctrl_dev *pctldev, unsigned int pin_id, unsigned long *config)
{
	struct nvt_pinctrl *npctl = pinctrl_dev_get_drvdata(pctldev);
...
static int nvt_pinconf_set(struct pinctrl_dev *pctldev, unsigned int pin_id,
...
static int nvt_pinctrl_parse_groups(struct device_node *np, struct nvt_pin_group *grp,
				    struct nvt_pinctrl *npctl, u32 index)
{
	...
	grp->npins = size / 4;
	grp->pins = devm_kzalloc(npctl->dev, grp->npins * sizeof(unsigned int), GFP_KERNEL);
	grp->settings = devm_kzalloc(npctl->dev, grp->npins * sizeof(struct nvt_pin_setting),
					GFP_KERNEL);
	if (!grp->settings)
		return -ENOMEM;
	...
	for (i = 0, j = 0; i < size; i += 4, j++) {
		...
		ret = pinconf_generic_parse_dt_config(np_config, NULL, &pin->configs,
						      &pin->nconfigs);
...
static int nvt_pinctrl_parse_functions(struct device_node *np, struct nvt_pinctrl *npctl,
					u32 index)
{
...
static int nvt_pinctrl_probe_dt(struct platform_device *pdev, struct nvt_pinctrl *npctl)
{
	...
	npctl->functions = devm_kzalloc(&pdev->dev, npctl->nfunctions * sizeof(struct nvt_pin_func),
					GFP_KERNEL);
	if (!npctl->functions)
		return -ENOMEM;

	npctl->groups = devm_kzalloc(&pdev->dev, npctl->ngroups * sizeof(struct nvt_pin_group),
					GFP_KERNEL);
	if (!npctl->groups)
		return -ENOMEM;
...
int nvt_pinctrl_probe(struct platform_device *pdev, const struct nvt_pinctrl_soc_info *info)
{
	...
	nvt_pinctrl_desc = devm_kzalloc(&pdev->dev, sizeof(*nvt_pinctrl_desc), GFP_KERNEL);
	if (!nvt_pinctrl_desc)
		return -ENOMEM;
	...
	ret = devm_pinctrl_register_and_init(&pdev->dev, nvt_pinctrl_desc, npctl, &npctl->pctl);
	if (ret) {
