                             ...
                             const struct pwm_state *state)
{
	...
	if (state->enabled) {
		...
		writel((readl(nvtpwm->base + MA35D1_REG_PWM_WGCTL0) & ~(WGCTL_MASK << (ch*2)))
		       | (WGCTL_HIGH << (ch*2)), nvtpwm->base + MA35D1_REG_PWM_WGCTL0);
		writel((readl(nvtpwm->base + MA35D1_REG_PWM_WGCTL1) & ~(WGCTL_MASK << (ch*2)))
		       | (WGCTL_LOW << (ch*2)), nvtpwm->base + MA35D1_REG_PWM_WGCTL1);
		/* Enable counter */
...
static int nuvoton_pwm_probe(struct platform_device *pdev)
{
	...
	chip->npwm = MA35D1_PWM_TOTAL_CHANNELS;
	//chip->ops = &nuvoton_pwm_ops;
