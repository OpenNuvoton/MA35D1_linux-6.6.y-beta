...
/*
 * Nuvoton MA35D0 Clock PLL driver
 *
...
#define to_ma35d0_clk_pll(clk) \
	(container_of(clk, struct ma35d0_clk_pll, clk))
...
static const struct vsipll_freq_conf_reg_tbl ma35d0pll_freq[] = {
...
static void CLK_UnLockReg(struct ma35d0_clk_pll *pll)
{
	int ret;
...
static void CLK_LockReg(struct ma35d0_clk_pll *pll)
{
	/* Lock PLL registers */
...
/* SMIC PLL for CAPLL */
unsigned long CLK_GetPLLFreq_SMICPLL(struct ma35d0_clk_pll *pll,
		...
		unsigned long u64PllFreq, u32 *u32Reg)
{
	...
	if (!((u64PllFreq >= VSIPLL_FCLKO_MIN_FREQ)
		...
		u32Reg[0] = ma35d0pll_freq[0].ctl0_reg;
		u32Reg[1] = ma35d0pll_freq[0].ctl1_reg;
		u64PllClk = ma35d0pll_freq[0].freq;
...
unsigned long CLK_GetPLLFreq_VSIPLL(struct ma35d0_clk_pll *pll,
		unsigned long PllSrcClk)
{
...
static int ma35d0_clk_pll_set_rate(struct clk_hw *hw, unsigned long rate,
		unsigned long parent_rate)
{
	struct ma35d0_clk_pll *pll = to_ma35d0_clk_pll(hw);

	if ((parent_rate < VSIPLL_FREF_MIN_FREQ) || (parent_rate > VSIPLL_FREF_MAX_FREQ))
		return 0;

	if ((pll->type == MA35D0_CAPLL) || (pll->type == MA35D0_DDRPLL)) {
		pr_warn("Nuvoton MA35D0 CAPLL/DDRPLL Read Only.\n");
		return 0;
	}
...
static unsigned long ma35d0_clk_pll_recalc_rate(struct clk_hw *hw,
			unsigned long parent_rate)
{
...
static long ma35d0_clk_pll_round_rate(struct clk_hw *hw, unsigned long rate,
					unsigned long *prate)
{
	return rate;
}

static int ma35d0_clk_pll_is_prepared(struct clk_hw *hw)
{
	struct ma35d0_clk_pll *pll = to_ma35d0_clk_pll(hw);
...
static int ma35d0_clk_pll_prepare(struct clk_hw *hw)
{
	struct ma35d0_clk_pll *pll = to_ma35d0_clk_pll(hw);
	u32 val;

	if ((pll->type == MA35D0_CAPLL) || (pll->type == MA35D0_DDRPLL)) {
		pr_warn("Nuvoton MA35D0 CAPLL/DDRPLL Enable Only.\n");
		return 0;
	}
...
static void ma35d0_clk_pll_unprepare(struct clk_hw *hw)
{
	struct ma35d0_clk_pll *pll = to_ma35d0_clk_pll(hw);
	u32 val;

	if ((pll->type == MA35D0_CAPLL) || (pll->type == MA35D0_DDRPLL))
		pr_warn("Nuvoton MA35D0 CAPLL/DDRPLL Cannot disable.\n");
	else {
		val = __raw_readl(pll->ctl1_base);
		val |= VSIPLLCTL1_PD_MSK;
		__raw_writel(val, pll->ctl1_base);
...
static const struct clk_ops ma35d0_clk_pll_ops = {
	.is_prepared = ma35d0_clk_pll_is_prepared,
	.prepare = ma35d0_clk_pll_prepare,
	.unprepare = ma35d0_clk_pll_unprepare,
	.set_rate = ma35d0_clk_pll_set_rate,
	.recalc_rate = ma35d0_clk_pll_recalc_rate,
	.round_rate = ma35d0_clk_pll_round_rate,
};

struct clk_hw *ma35d0_reg_clk_pll(enum ma35d0_pll_type type,
				u8 u8mode, const char *name,
				const char *parent, unsigned long targetFreq,
				void __iomem *base, struct regmap *regmap)
{
	struct ma35d0_clk_pll *pll;
	struct clk_hw *hw;
