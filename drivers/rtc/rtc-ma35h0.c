...
static inline unsigned int rtc_reg_read(struct ma35h0_rtc *p,
									int offset)
{
	return __raw_readl(p->rtc_reg + offset);
}

static inline void rtc_reg_write(struct ma35h0_rtc *p,
...
static int *check_rtc_access_enable(struct ma35h0_rtc *ma35h0_rtc)
{
	rtc_reg_write(ma35h0_rtc, REG_RTC_INIR, INIRRESET);

	mdelay(1);

	rtc_reg_write(ma35h0_rtc, REG_RTC_AER, AERPOWERON);
...
static int ma35h0_rtc_bcd2bin(unsigned int timereg,
...
static int ma35h0_rtc_alarm_bcd2bin(unsigned int timereg,
...
static int ma35h0_ioctl(struct device *dev, unsigned int cmd,
						unsigned long arg)
{
	struct ma35h0_rtc *rtc = dev_get_drvdata(dev);
...
{
	struct resource *res;
	struct ma35h0_rtc *ma35h0_rtc;
	struct clk *clk;
	u32 val32;
	int err;

	ma35h0_rtc = devm_kzalloc(&pdev->dev, sizeof(struct ma35h0_rtc),
								GFP_KERNEL);
	if (!ma35h0_rtc)
		return -ENOMEM;

	res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	ma35h0_rtc->rtc_reg = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(ma35h0_rtc->rtc_reg))
		return PTR_ERR(ma35h0_rtc->rtc_reg);
	...
	platform_set_drvdata(pdev, ma35h0_rtc);

	ma35h0_rtc->rtcdev = devm_rtc_device_register(&pdev->dev,
							pdev->name, &ma35h0_rtc_ops, THIS_MODULE);
	if (IS_ERR(ma35h0_rtc->rtcdev)) {
		dev_err(&pdev->dev, "rtc device register failed\n");
		return PTR_ERR(ma35h0_rtc->rtcdev);
	}

	rtc_reg_write(ma35h0_rtc, REG_RTC_TSSR,
		(__raw_readl(ma35h0_rtc->rtc_reg + REG_RTC_TSSR) | MODE24));

	ma35h0_rtc->irq_num = platform_get_irq(pdev, 0);

	if (devm_request_irq(&pdev->dev, ma35h0_rtc->irq_num,
			ma35h0_rtc_interrupt, IRQF_NO_SUSPEND, "ma35h0rtc", ma35h0_rtc)) {
		dev_err(&pdev->dev, "ma35h0 RTC request irq failed\n");
		return -EBUSY;
	}

	ma35h0_rtc->LXT_Fail = 0;
	...
	if (val32 == 1) {
		ma35h0_rtc->LXT_Detect = 1;
		rtc_reg_write(ma35h0_rtc, REG_RTC_CDBR, 0x800080);
		rtc_reg_write(ma35h0_rtc, REG_RTC_CLKDCTL,
			(__raw_readl(ma35h0_rtc->rtc_reg + REG_RTC_CLKDCTL) | 0x1));
		rtc_reg_write(ma35h0_rtc, REG_RTC_RIER,
			(__raw_readl(ma35h0_rtc->rtc_reg + REG_RTC_RIER) |
			TICKINTENB | CLKSTIEN | CLKFIEN | PSWIEN));
	} else {
		ma35h0_rtc->LXT_Detect = 0;
		rtc_reg_write(ma35h0_rtc, REG_RTC_CDBR, 0x0);
		rtc_reg_write(ma35h0_rtc, REG_RTC_CLKDCTL,
			(__raw_readl(ma35h0_rtc->rtc_reg + REG_RTC_CLKDCTL) & ~0x1));
		rtc_reg_write(ma35h0_rtc, REG_RTC_RIER,
			(__raw_readl(ma35h0_rtc->rtc_reg + REG_RTC_RIER) | TICKINTENB | PSWIEN));
	}
...
{
	struct ma35h0_rtc *ma35h0_rtc = platform_get_drvdata(pdev);

	if (device_may_wakeup(&pdev->dev))
		enable_irq_wake(ma35h0_rtc->irq_num);

	rtc_reg_write(ma35h0_rtc, REG_RTC_RIER,
		(__raw_readl(ma35h0_rtc->rtc_reg + REG_RTC_RIER) & ~TICKINTENB));
...
static int ma35h0_rtc_resume(struct platform_device *pdev)
{
	...
	if (device_may_wakeup(&pdev->dev))
		disable_irq_wake(ma35h0_rtc->irq_num);
...
static struct platform_driver ma35h0_rtc_driver = {
	...
	.driver		= {
		.name	= "ma35h0-rtc",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(ma35h0_rtc_of_match),
	},
};
