...
#include <uapi/misc/ma35d0_ebi.h>
#include "regs-ma35d0-ebi.h"
...
static inline void ma35d0_set_ebi_ctl(u8 u8Bank)
{
...
{
	struct ma35d0_set_ebi *pebi, sebi;
...
static int ebi_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (remap_pfn_range(vma, vma->vm_start, pageFrameNo, size, vma->vm_page_prot)) {
		pr_err("ma35d0_mem_mmap() : remap_pfn_range() failed !\n");
		return -EINVAL;
	}
...
static int ma35d0_ebi_probe(struct platform_device *pdev)
{
	unsigned int ch;
	int ret;
	...
	ma35d0_set_ebi_ctl(ebi[ch]->bank);
...
static int ma35d0_ebi_remove(struct platform_device *pdev)
{
...
static const struct of_device_id ma35d0_ebi_of_match[] = {
	{ .compatible = "nuvoton,ma35d0-ebi" },
	{},
};
MODULE_DEVICE_TABLE(of, ma35d0_ebi_of_match);

static struct platform_driver ma35d0_ebi_driver = {
	.driver = {
		.owner  = THIS_MODULE,
		.name = "ma35d0-ebi",
		.of_match_table = of_match_ptr(ma35d0_ebi_of_match),
	},
	.probe = ma35d0_ebi_probe,
	.remove = ma35d0_ebi_remove,
};

module_platform_driver(ma35d0_ebi_driver);

MODULE_ALIAS("platform:ma35d0-ebi");
MODULE_DESCRIPTION("EBI driver for Nuvoton MA35D0");
